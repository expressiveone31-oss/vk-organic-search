...
from bot.utils.similarity import (
    contains_phrase,
    contains_phrase_with_gap,
    match_score,
    find_match_window,
)
...
def _telemetr_body(it: dict) -> str:
    parts = []
    for key in ("title", "text", "caption", "message", "description", "html_text", "text_html", "body", "content", "plain_text"):
        v = it.get(key)
        if v:
...
async def _search_telemetr(seed: str, since: dt.date, until: dt.date) -> Tuple[List[Publication], List[str]]:
    ...
    for page in range(TELEMETR_PAGES):
        ...
        for it in items:
            ...
            if TELEMETR_STRICT:
                ok = contains_phrase(seed, body) or contains_phrase_with_gap(seed, body, max_gap_words=TELEMETR_MAX_GAP_WORDS)
            else:
    ...
    diags.append(f"Telemetr total≈{total}, matched={matched}, strict={TELEMETR_STRICT}, gap={TELEMETR_MAX_GAP_WORDS}, thr={TELEMETR_FUZZY_THRESHOLD}")
    return pubs, diags
...
async def _search_tgstat(seed: str, since: dt.date, until: dt.date) -> Tuple[List[Publication], List[str]]:
    ...
    if (data or {}).get("status") != "ok":
        return pubs, [f"TGStat error: {data}"]
    ...
    for it in items:
        ...
        if not contains_phrase_with_gap(seed, body, max_gap_words=TELEMETR_MAX_GAP_WORDS):
            continue
